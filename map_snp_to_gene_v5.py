...
import os, math, traceback, datetime, urllib, requests, json, argparse, sys, shutil

def set_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("-f", help ="name of the csv file outputed by gapit.")
    parser.add_argument("-p", help="logP_threshold for SNPs", type=int)
    parser.add_argument("-sp", help="common name of species subjected to gwas")
    args = parser.parse_args()


def resformat(filter):
    filter = "{}Results_filtered.txt".format(args.f)
...
def getgs(genedesign):
    ...
    with open(genedesign, "r") as gk:
        with open("genome.json", "w") as af:
            ...
            url = "http://knetminer.rothamsted.ac.uk/{}/genome?keyword={}&list={}".format(species, keyw, genelist)
            print(url)
            r = requests.get(url)
            r.json()
    ...
    with open(genetable, "r") as f:
        ...
        with open(scores, "w") as sf:
            for line in f:
                ...
                link="http://knetminer.rothamsted.ac.uk/{}/genepage?".format(species)
...
def main():
    ...
    threshfile = "{}Results_filtered_threshold.txt".format(args.f)
    ...
    #get rid of annotation by uploading it from ensembl
    #Ask David if he has the allele information contained in snp effect table.
    #whether gapit should be re-run.

    
    #4) Obtain a file summarising the information.
    gdp = "{}Results_filtered_gdp_FINAL.txt".format(args.f)
    genedesign = "{}Results_filtered_gene_and_designation.txt".format(args.f)
    ...
    genetable="{}genetable.txt".format(args.f)
    try:
        parsejs(genetable)
    ...
    genetable="{}genetable.txt".format(args.f)
    scores="{}scores.txt".format(args.f)
    try:
...
#repr will print a string with "" around it.
